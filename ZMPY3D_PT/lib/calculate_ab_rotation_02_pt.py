...
def calculate_ab_rotation_02_pt(z_moment_raw, target_order_2_norm_rotate):
    ...
    f20 = z_moment_raw[2, 2, 0].real
    f21 = z_moment_raw[2, 2, 1]
    f22 = z_moment_raw[2, 2, 2]
    f21_im = f21.imag
    f21_re = f21.real
    f22_im = f22.imag
    f22_re = f22.real

    coef4 = (4 * f22_re * k_im * (-1 + k_im2 - 3 * k_re2) - 4 * f22_im * k_re * (1 - 3 * k_im2 + k_re2)
             - 2 * f21_re * k_im * k_re * (-3 + k_im2 + k_re2) + 2 * f20 * k_im * (-1 + k_im2 + k_re2)
             + f21_im * (1 - 6 * k_im2 + k_im2 ** 2 - k_re2 ** 2))
    
    coef3 = (2 * (-4 * f22_im * (k_im + k_im3 - 3 * k_im * k_re2) + f21_re * (-1 + k_im4 + 6 * k_re2 - k_re4)
             + 2 * k_re * (f22_re * (2 + 6 * k_im2 - 2 * k_re2) + f21_im * k_im * (-3 + k_im2 + k_re2)
             + f20 * (-1 + k_im2 + k_re2))))
